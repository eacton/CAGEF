...
##Quick Intro to the Intro to R Lesson Series##
...
This lesson is the first in a 6-part series. The idea is that at the end of the series, you will be able to import and manipulate your data, make some exploratory plots, perform some basic statistical tests as well as regression, and make some even prettier plots and documents to share your results. So that is where we are going.
...
How do we get there? Today we are going to be learning about the basic data structures in R, how to get your data in and out of R, get cozy with the R environment, and how to get help when you are stuck. Because everyone gets stuck. A lot. Next week we will learn how to tidy our data, subset and merge data and get some descriptive statistics. The next lesson will be data cleaning and string manipulation, because this is really the battleground of coding - getting your data into the format where you can analyse it. This is where the uncensored real world data examples come in. After that, we will make all sorts of plots - from simple data exploration to interactive plots - this is always a fun lesson. And then lastly, we will learn to write some functions, which really can save you time and help scale up your analyses.
...
R is an 'environment' because it has the tools and software for the storage, manipulation, statistical analysis, and a means of graphical display for data.  It comes with about 25 'packages', and uses a simple programming language ("S") and users have been encouraged to make there own packages... there are now over 10,000 packages on CRAN and about 1,500 on Bioconductor.
...
R will tell you your _absolute directory_. An absolute directory is a path starting from your root ("/"). The absolute directory can vary from computer to computer, as my home directory and your home directory are not the same.


To move directories, it is good to know a couple shortcuts. `'.'` is your current directory. `'..'` is up one level. `'~'` is your home directory (a shortcut for "home/sue"). Therefore, our current location could also be denoted as `"~/subdirectory1"`.

To move to the directory 'Sue' we could type: `"/home/sue"` or `"~"` or `".."`.

A _relative directory_ is a path starting from whereever your currently are. This could be the same on your computer and my computer if and only if we have the same directory structure. 

If I wanted to move back to subdirectory1 using the _absolute path_, I would use a function that will set my working directory.
...
Your worst collaborator is actually you in 6-months.     
...
- Put a description of what you are doing near your code (I prefer above) - at every process, decision point, or non-default argument in a function. For example, to re-run your analysis in 6-months and you think you will remember why you selected k=6, or the Spearman over Pearson option for your correlation matrix, or quantile over median normalization, but you won't. 
 - Give your objects informative object names __that are not the same as function names__. 

Key-board shortcuts:

Code -  Comment/Uncomment lines CTRL + SHIFT + C
Code - Reflow Comment (Wrap comments) CTRL + SHIFT + /
...
- working directory  - can't find files - use getwd() to check where you are working, the Files window to check that your file exists there, and setwd() to change your directory if it doesn't
                         - work inside an R project with your files in the same folder - your working directory will be set automatically
- typos - R is case sensitive, check that you've spelled everything right
            - use tab-completion when possible
- open quotes, parentheses, brackets
            - R Studio highlights matching brackets, and gives an angry red sidebar if your final bracket is not closed
- data type - you can't perform a calculation on those 1, and 0's because they are actually characters and not numeric
- unexpected answers - check in the help menu ie. help("function") that the function works like you think it does
- function is not found but I have installed the package - you still have to load the package
...
- I got the R bomb!! - congrats, rite of passage. You are doing something R doesn't want to do. Consult online.
    
_Beginner Advice_ Try to solve a problem yourself, but set a 30 minute cutoff on being stuck. At this level, people have had and solved your problem. Many beginners get frustrated because they get stuck and take hours to solve a problem themselves. Set your limit, stay within it, then go online and get help.

- Finding answers online
  + 99% of the time, someone has already asked your question
  + Google, Stackoverflow, seqanswers, quora, researchgate, even reddit, twitter
  + Including the program, version, error, and package helps, be specific
  + http://stat545.com/help-general.html #this is fun - show class
...
- Remember - with programming, there are multiple ways to come up with an answer. Just because your code doesn't look the same as someone else's code, doesn't necessarily mean that it is wrong. There are different packages that let you do the same thing, but with shortcuts. There are different levels of 'efficiency' and 'redundancy' in coding. You will work on refining these aspects of your code as you go along in this course, and in your coding career.

- Under Help there is a Cheatsheet of Keyboard Shortcuts.
...
Let's look up the function c(), which combines values into a vector or list. It takes in the argument '...', which is defined as objects to be concatenated. In the description it says 'all agruments are coerced to a common type, which is the type of the returned value' and in the details it tells you the hierarchy 'NULL < raw < logical < integer < double < complex < character < list < expression' and 'factors are treated only via their internal integer codes'. Now we don't know what all this means yet, but we come back to this as we go along. The output of the function is 'NULL or an expression or a vector'. At the bottom of the help menu, there are some examples of how to use the function, which are sometimes easier to understand than the documentation. 
...
```
Some package creaters will have created their own data classes and will require your data to be in the format required of that class. For example in Bioconducter there is an ExpressionSet class. 

![](img/expressionset.jpg)

</br>

This class contains your metadata, your assay data, and your feature data (genes, probes, whatnot) in different slots. Functions in a package may not work unless your data is in the ExpressionSet class format. This isn't something we will be dealing with a lot in this class, but it is good to be aware of from a trouble-shooting perspective. 
