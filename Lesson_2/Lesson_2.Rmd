...
To help us be able to answer these questions, we are going to learn the main functions in the `dplyr()` and `tidyr()` packages. At the same time, I am going to show you how to subset in base R (ie. without a fancy package that makes your life easier). The reason that I am doing this is because while a lot of new packages play nicely with the functions we are going to use today, not all packages or data structures will. However, these functions are so intuitive and save so much time that they are worth learning.
...
It is often extremely useful to subset your data by some logical condition (`==` (equal to), `|` (or)). In this case, we want to keep all rows that have either Fusobacteria or Methanobacteria.
