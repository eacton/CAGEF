...
#subsetting
dat[1:5,]
#technically, but fallible, not obvious if following code, and so much typing
...
```{r include = FALSE}
dat %>% select(contains("_2_")) 

dat %>% select(matches("_2_")) 

dat %>% select(starts_with("T_2_"), starts_with("V_2_")) 
...
`n()` is a `dplyr` function to count the number of observations in a group - we are simply going to count the instances of a Taxa. We can arrange the results in descending order to see if there is more than one row per Taxa. Can anyone think of another way to see if there is more than one row per Taxa? 
...
ndat <-  spread_dat %>% filter(OTUs!=0) %>% arrange(desc(OTUs)) %>% .[1:20, ]
```
