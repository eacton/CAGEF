...
I haven't yet removed the tags. With the `str_replace` function we can specify our pattern and replacement, in this case we can replace the tags with an empty character string. We can see in the result that the tags have been replaced.

```{r}
str_replace_all(elon_tweets_df$text[1:50], pattern = tags, replacement = "")
...
`str_remove` is similar to `str_replace`, but it has the default replacement set to an empty character string. Check to see if your regular expression is optimized on a large portion of your data before saving your cleaned output. 
...
...another challenge question on how to do this all at once?
