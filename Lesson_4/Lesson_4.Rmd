...
Notice that this point is a huge outlier, and has changed the output of our plot. However, our summary also depends on the cars dataset and has not been updated (ie. the maximum distance is still 120 km). The code of the cached chunk did not change and was not recalculated.
...
Since the code in the cache changed, its values were recalculated. Changes in the cache are still evaluated and passed on to the next code block. They are still being stored. The plot does now has a point at dist = 300, the the max distance in the summary has changed.

If I change cars (with the outlier point) back to cars (without the outlier point) and knit the document, what do you expect to happen?
...
Since the code for our cached chunk doesn't change, it does not update and the plot, downstream of our cache does not reflect the above change.

What if we comment the outlier point in the cached chunk instead? 
...
The code in the cache changed and so the summary was recalcuated using earlier chunks. Both the summary and the plot show the 200 km distance value.
...
Since the code in the cached chunk did not changed, it accesses the cached data files and the summary will have 200 km as max. However, the plot needs to be created from scratch and uses cars from the first chunk so has a max of 120 km.
...
The cache files are kept on the computer. The summary table reflects the prevously cached value of 200 km, the plot is calculated from the first chunk and has a max of 120 km. 
