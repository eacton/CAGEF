...
kable(text_table, "html", caption = "Matching by Position") %>%
...
kable(text_table, "html", caption = "Quantifiers") %>%
  kable_styling(full_width = F) %>%
  column_spec(1, italic = T, border_right = T) %>%
  column_spec(2, width = "40em")
```
...
kable(text_table, "html", caption = "Classes") %>%
  kable_styling(full_width = F) %>%
  column_spec(1, italic = T, border_right = T) %>%
  column_spec(2, width = "40em")
```
...
kable(text_table, "html", caption = "Operators") %>%
  kable_styling(full_width = F) %>%
  column_spec(1, italic = T, border_right = T) %>%
  column_spec(2, width = "40em")
```
...
kable(text_table, "html", caption = "Escape Characters") %>%
  kable_styling(full_width = F) %>%
  column_spec(1, italic = T, border_right = T) %>%
  column_spec(2, width = "40em")
```
...
str_replace_all(trump_tweets_df$text[1:20], clean5, replacement = "")
...
trump_tweets_df$text <- str_remove_all(trump_tweets_df$text, pattern = clean5)
...
For the code chunk containing `plot(pressure)`: How would you show just the code and not run the code?  How would you show just the code and have the output run but not show it? How would you change the background color to something other than gray? You can the help pages or the knitr documentation found here: <https://yihui.name/knitr/options/#chunk_options> if you need to.
...
__I think knitr loaded at the beginning can work here__
...
Add footnotes. __Maybe we don't want eval = F here __
