...
```
But let's take a second to look at the alternative.
...
ggplot provides 2 methods of labelling. Let's look at the help menu to find out what the difference between the 2 of them is. We have to specify in with aesthetics, that we are plotting our label. 
...
p
```
A common thing to want to do is to change colours from ggplot's rainbow color scheme. Let's create our own colour palette for Taxa.
...
Which of these is ggplot's default color palette?
...
ggplot(dat[dat$Taxa %in% keep,] , aes(x = Taxa, y = OTUs, fill = Taxa)) + 
  geom_boxplot(outlier.colour = "red") + 
  theme_minimal() +
  theme(axis.text.x = element_text(angle =90, hjust=1), 
...
ggvis is a Bioconductor package allowing interactive graphics in R (http://ggvis.rstudio.com/). It is founded on the grammar of graphics and the syntax is similar, but not identical to ggplot. It is good for interactive scatterplots, bar graphs, line graphs and histograms (although not heatmaps at this point in time). The nature of the interactiveness is highly customizable: you can hover, add slider bars, checkboxes or radiobuttons (it uses Shiny's reactive programming).
...
##Taking it up a notch_ - Maybe preview these, let's do one heatmap in plotly and one phylogenetic tree
